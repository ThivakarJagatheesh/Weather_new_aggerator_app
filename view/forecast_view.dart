...
class ForecastViewState extends ConsumerState<ForecastView> {
  ...
  void didChangeDependencies() {
    temperatureUnit = TemperatureProvider.of(context)!.unit;
   debugPrint("uri encode is ${ Uri.encodeComponent(
        "People keep asking Jodie Humphries when she'll have kids as she nears 40. She's still happy being child-free and knows she won't regret her choices.")
   }");
            ...
            return ListView.separated(
                ...
                itemBuilder: (_, index) {
                  if (temperatureUnit == TemperatureUnit.celsius) {
                    temperature =
                        Temperature.kelvin(data[index].temperature).celsius;
                  } else {
                    temperature =
                        Temperature.kelvin(data[index].temperature).farhenheit;
                  }
                  return ListTile(
                    title: Text(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        Text("Temp: "
                            "${temperature.roundToDouble()} ${temperatureUnit == TemperatureUnit.celsius ? "\u2103" : "F"}"),
                        Text(
